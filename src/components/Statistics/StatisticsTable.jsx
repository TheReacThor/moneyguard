...
import { selectSummary, selectStatLoading } from '../../redux/Statistics/selectors';
...
const StatisticsTable = () => {
  ...
  const expenses = Array.isArray(summary) ? summary.filter(item => item.type === 'EXPENSE') : [];
  const income = Array.isArray(summary) ? summary.filter(item => item.type === 'INCOME') : [];

  // Toplam harcama ve gelir hesapla
  const totalExpenses = expenses.length > 0 
    ? expenses.reduce((acc, item) => acc + Math.abs(parseFloat(item.total)), 0) 
    : 0;
